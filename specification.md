...
```bash
# Generate the private key used to sign attestations
...
If the project defines its build in a Makefile, a Makefile target can be wrapped instead. In that case, the attestation records the activity of every command that the target runs.

```bash
# Makefile target that runs the build
make build
```
