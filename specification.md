...
### 6.1 Overview
This chapter describes how attestations are actually generated. For what an attestation is expected to contain, see 5.1.1.

SBOMit assumes that evidence is collected while supply chain steps are being performed. This requires a tool that can observe the execution of a supply chain step and emits observations as an in-toto attestation, signed by the key of the functionary that performed the step.

[witness](https://github.com/in-toto/witness) is an implementation that currently fills this role. The descriptions in this chapter treat it as a reference implementation. The SBOMit specification, however, does not depend on witness. Any other implementation that provides the behavior described above are equally as compatible.

An attestation records only the activity that occurs within the execution of the wrapped command. Anything that happens outside that scope does not appear in the attestation. Choosing which command is observed determines the completeness of the collected evidence.

### 6.2 Capturing the Required Evidence
The attestation generated by witness has a layered structure. The outermost layer is a DSSE envelope, which carries the signature. Inside the envelope is an in-toto Statement, which describes the subject of the attestation. The Statement contains an attestation collection. The collection holds the results that multiple attestors gathered during a single execution of the wrapped command. This section describes the evidence in the attestation collection that is used for SBOM generation.

- Materials (material attestor): This is a snapshot of the working directory taken immediately before the wrapped command runs. Each file is recorded as a pair of its path and its digest.

- File access (command-run attestor): This is a record of the files accessed by processes that the eBPF tracer observed during command execution. It is stored in the processes field of the command-run attestor as the openedfiles of each process. The record also includes files accessed outside the working directory. For example, paths that contain the pkg/mod/ path segment of the Go module cache are recorded here. SBOMit identifies such files as packages based on these path patterns.

- Network access (command-run attestor): This is a record of the network access that the eBPF tracer observed during command execution. It is not produced by a separate attestor. It is recorded inside the command-run attestor together with file access. Network access does not add new packages. Instead, it attaches URL and IP information to packages that have already been identified through file access.

- Products (product attestor): This is the result of comparing a snapshot of the working directory taken immediately after the wrapped command runs with the materials.

All of this evidence is limited to activity observed within the attestation boundary described in 6.1. File access or network access that occurs outside the wrapped command is not recorded in the attestation.

### 6.3 Delegated Builds

### 6.4 Containerized Builds

### 6.5 Example of Generating an Attestation
This section shows an example of generating an attestation with witness. The example only illustrates how an attestation is generated. The Go project wrapped in this example is also a hypothetical project used to aid understanding.

#### Prerequisites

The following are required to run the example.

* Linux kernel: The eBPF tracer in witness requires Linux kernel 6.12 or later.
* witness binary: The `witness` command must be installed and executable.
* Signing key: witness signs every attestation it generates with the key of the functionary. The signature shows who generated the attestation.

Build tools in most ecosystems keep a local cache of downloaded dependencies. If a dependency is already in the cache, the build does not download it again. In that case, network access for that dependency may not be recorded in the attestation. Clearing the cache is not required. However, clearing the dependency cache before generating the attestation is recommended to record network access more accurately.

#### Choosing the command to wrap

Choosing the command to wrap determines the attestation boundary. As described in 6.1, an attestation records only the activity that occurs during the execution of the wrapped command. The user should wrap the build command that covers the scope they want to record in the attestation. This example wraps the following command.

```bash
# Build command of the example project
go build -o bin/example ./cmd/example
```

witness does not determine whether a build succeeds or fails. The result is determined by the wrapped command itself. witness only observes its execution. Therefore, a command that succeeds without witness also succeeds when wrapped with witness, as long as the prerequisites and the steps below are completed.

#### Running witness

Generating an attestation in this example consists of two steps.

**1. Generate and trust the interception CA**

The `network-trace` attestor records network access by placing a MITM proxy between the build and the network. Most dependency downloads use HTTPS. An HTTPS client accepts a connection only if the server certificate is signed by a CA that the client trusts. The proxy presents certificates signed by its own CA. If this CA is not trusted, the build tools reject the connection and every HTTPS request fails. For this reason, the CA must be registered in the system trust store before the build runs.

witness generates this CA the first time the `network-trace` attestor runs. The following command runs witness once only to generate the CA. The wrapped command `/bin/true` does nothing. The attestation from this run is not needed, so it is written to `/dev/null`.

```bash
# Generate the CA. The wrapped command does nothing.
sudo -E HOME=$HOME PATH=$PATH witness run \
  --experimental \
  --step ca-bootstrap \
  -a network-trace \
...
The CA certificate is created in `./witness_nettrace_proxy/`. The following commands register it in the system trust store.

```bash
# Copy the CA certificate to the system CA directory
sudo cp ./witness_nettrace_proxy/ca_cert.pem \
  /usr/local/share/ca-certificates/witness-nettrace.crt

# Update the system trust store to include the new CA
sudo update-ca-certificates
```

Run every subsequent step from this same directory. The default paths of the CA certificate and key are `./witness_nettrace_proxy/`, relative to the current directory. If witness runs from another directory, it generates a new CA that is not in the system trust store.

**2. Record the build attestation**

The following command wraps the build command and records the attestation.

```bash
# Wrap the build command with witness and record the attestation
sudo -E HOME=$HOME PATH=$PATH witness run \
  --step build \
  --experimental \
  -a network-trace \
...
* `--step build`: The name of the supply chain step that this attestation records.
* `--experimental`: Enables the `network-trace` attestor.
* `-a network-trace`: Adds the attestor that records network access.
* `--trace --attestor-command-run-trace-backend ebpf`: Uses the eBPF tracer to record the files opened during the build.
* `--signer-file-key-path ./signing-key.pem`: The path of the key used to sign the attestation.
* `-o attestation.json`: The path where the attestation is written.
* Everything after `--`: The build command to wrap. To wrap a Makefile target, replace it with the target, for example `make build`.

When the command finishes, `attestation.json` contains the signed attestation.
...
The materials and products recorded in link metadata, and the artifacts observed by trace attestations, are paths accompanied by secure hashes. An SBOM is expressed in terms of packages and files. Artifact resolution is needed to bridges the two. Resolution proceeds in three stages, in order:

1. **Exclusion.** Artifacts whose paths identify them as properties of the build machine rather than of the software are discarded and play no further part.
2. **Package resolution.** Artifacts whose paths or network provenance establish a package identity are grouped into package entries, and the files belonging to each resolved package are attributed to it.
3. **Remainder.** Artifacts that survive both stages are recorded as file entries.

#### 7.2.1 Excluded paths

An artifact is excluded when its path shows it to be a property of the environment in which the build ran. Such artifacts are not absent from the record but they are not components of the software and do not belong in the SBOM. Five categories are excluded:
...
A package entry requires a resolvable identity: an ecosystem, a name, and a version, together sufficient to create a package URL. The PURL is what allows a component derived from attestations to be recognized as the same component when it also appears in supplemental SBOM information or in output from a conventional scanner, and it is therefore the identity under which entries are deduplicated and merged.

This gives the criterion that separates a package from a file. Package managers record their installations in path layouts, and those layouts fall into two classes: layouts that encode both the name and the version of what they contain, and layouts that encode only the name. Only the first category yields a complete identity from path evidence alone. In the second, the version lives outside the path (sometimes in a lockfile whose *content* the attestation does not capture). Artifacts observed on the filesystem are resolved from the following layouts:
...
Artifacts may also be resolved from network provenance. Where a attestation records the connections a build made, each connection is attributed to the ecosystem that owns its destination host, and the requests within it are matched against that registry's URL layout. Network evidence additionally establishes where a component came from: the request URL and the address it resolved to are retained as download provenance.
...
Artifacts that are neither excluded nor resolved into a package are recorded as files. This is the expected outcome for the largest part of a typical build. This includes the project's own source tree, its tests, documentation, and generated assets are components of the software, are correctly described at file granularity, and have no package identity to resolve.
